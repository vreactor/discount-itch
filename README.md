...
The discount-itch package (_ditch for short_) is a package that exposes methods to get all items on 100% discount of itch.io.

## How to use

### Get the package

`go get -u github.com/VReactor/discount-itch`
...
func main() {
    ...
    for _, category := range ditch.Categories {
        jsonString := ditch.GetCategoryItemsAsJSON(category)
        fmt.Println(jsonString)
    }

    // get all the games on 100% discount
    games, err := ditch.GetCategoryItems(ditch.Games)
	if err != nil {
		fmt.Println(err)
	} else {
